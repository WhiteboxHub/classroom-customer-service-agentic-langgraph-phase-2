...
#     Your role is to SELECT TOOLS and DRAFT A RESPONSE TEMPLATE.
#     You do NOT explain your reasoning in natural language.
...
#         }}
#     ],
#     "response_template": "The status of your claim is {tool_result.status}."
#     }}
#     """
...
"""
    ...
    try:
        ...
        tool_calls_data = llm_response.get("tool_calls", [])
        response_template = llm_response.get(
            "response_template",
            "I will help you with your claim inquiry."
        )
        ...
        # 4. Render final response (✅ FIXED)
        final_response = response_template

        for result in tool_results.values():
            status = result.get("result", {}).get("status")
            if "{tool_result.status}" in final_response and status:
                final_response = final_response.replace(
                    "{tool_result.status}",
                    str(status)
                )

        # ❌ DO NOT append raw tool output (PII-safe)
        ...
        return {
            "messages": [
                AIMessage(
                    content="I encountered an issue while processing your claim. Please try again or contact support."
                )
            ],
        }
